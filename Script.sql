create database StoryProdut


--SEXO = {CODSEXO, NOMESEXO}
create table sexo (
	codsexo INT NOT NULL IDENTITY PRIMARY KEY,
	nomesexo varchar(80) not null unique
);

insert into sexo(nomesexo) values (UPPER('MASCULINO'));
insert into sexo(nomesexo) values (UPPER('feminino'));


--RUA = {CODRUA, NOMERUA}
create table rua (
	codrua INT NOT NULL IDENTITY PRIMARY KEY ,
	nomerua varchar (80) not null unique
);

insert into rua(nomerua) values (upper('Rua 21'));


--BAIRRO = {CODBAIRRO, NOMEBAIRRO}
create table bairro(
	codbairro INT NOT NULL IDENTITY PRIMARY KEY,
	nomebairro varchar (80) not null unique
	
);

insert into bairro(nomebairro) values(upper('centro'));


-- CEP = {CODCEP, NUMEROCEP}
create table cep(
	codcep INT NOT NULL IDENTITY PRIMARY KEY,
	numerocep char(9) not null unique
);

insert into cep(numerocep) values('157750000')



--UF = {CODUF, NOMEUF, SIGLA}
create table uf (
    coduf INT NOT NULL IDENTITY PRIMARY KEY,
...
create table trabalho(
	codtrabalho INT NOT NULL IDENTITY PRIMARY KEY,
	nometrabalho varchar(80) not null unique
);



--CLIENTE = {CODCLIENTE, NOMECLIENTE, FOTO, DATANASC, CODSEXOFK, CODRUAFK,
--CODBAIRROFK, CODCEPFK, CODCIDADEFK, SALARIO, CODTRABALHOFK, NUMEROCASA}
create table cliente(
	codcliente INT NOT NULL IDENTITY PRIMARY KEY,
	nomecliente varchar(80) not null,
	foto varbinary(max) not null,
	datanasc date not null,
	codsexo_fk integer not null references sexo(codsexo)ON DELETE CASCADE ON UPDATE CASCADE,
	codrua_fk integer not null references rua(codrua)ON DELETE CASCADE ON UPDATE CASCADE,
	codbairro_fk integer not null references bairro(codbairro)ON DELETE CASCADE ON UPDATE CASCADE,
	codcep_fk integer not null references cep(codcep)ON DELETE CASCADE ON UPDATE CASCADE,	
	codcidade_fk integer not null references cidade(codcidade)ON DELETE CASCADE ON UPDATE CASCADE,
	salario numeric(8,2) not null check(salario > 0),
	codtrabalho_fk integer not null references trabalho(codtrabalho)ON DELETE CASCADE ON UPDATE CASCADE,
...
create table operadora (
	codoperadora INT NOT NULL IDENTITY PRIMARY KEY,
	nomeoperadora varchar (80) unique not null 
);

insert into operadora(nomeoperadora) values (upper('vivo'));
insert into operadora(nomeoperadora) values (upper('claro'));
insert into operadora(nomeoperadora) values (upper('tim'));
insert into operadora(nomeoperadora) values (upper('Oi'));

--TELEFONE = {CODTELEFONE, NUMERO, CODOPERADORAfk}
create table telefone (
	codtelefone INT NOT NULL IDENTITY PRIMARY KEY,
	numero varchar (11) not null unique,
	codoperadora_fk integer not null references operadora(codoperadora)ON DELETE CASCADE ON UPDATE CASCADE
);

insert into telefone(numero,codoperadora_fk) values('67992706066',2);


--ITENSTELEFONECLIENTE = {CODTELEFONEFK, CODCLIENTEFK}
create table itenstelefonecliente(
	codtelefone_fk integer not null references telefone(codtelefone)ON DELETE CASCADE ON UPDATE CASCADE,
	codcliente_fk integer not null references cliente(codcliente) ON DELETE CASCADE ON UPDATE CASCADE,
	primary key (codtelefone_fk, codcliente_fk)
);



--ITENSTELEFONETRABALHO = {CODTRABALHOfk, CODTELEFONEFK}
...
create table funcao(
	codfuncao INT NOT NULL IDENTITY PRIMARY KEY,
...
create table loja(
	codloja INT NOT NULL IDENTITY PRIMARY KEY,
...
values (upper('Sistema'), '52654486000152', upper('Sistema'), upper('sistema'));

--FUNCIONARIOS = {CODFUNCIONARIO, NOMEFUNCIONARIO, NUMEROCASA,
--CODRUAFK, CODBAIRROFK, CODCEPFK, CODCIDADEFK, CODFUNCAOFK, SALARIO, CODLOJAFK}
create table funcionario(
	codfuncionario INT NOT NULL IDENTITY PRIMARY KEY,
	...
	codcep_fk integer not null references cep(codcep) ON DELETE CASCADE ON UPDATE CASCADE,
	codcidade_fk integer not null references cidade(codcidade) ON DELETE CASCADE ON UPDATE CASCADE,
	codfuncao_fk integer not null references funcao(codfuncao) ON DELETE CASCADE ON UPDATE CASCADE,
	salario numeric (10,2) not null check(salario > 0),
	codloja_fk integer not null references loja(codloja) ON DELETE CASCADE ON UPDATE CASCADE
);
...
create table itenstelefonefuncionario(
	...
	codfuncionario_fk integer references funcionario(codfuncionario) ON DELETE CASCADE ON UPDATE CASCADE,
	primary key(codfuncionario_fk, codtelefone_fk)
);

insert into itenstelefonefuncionario(codtelefone_fk,codfuncionario_fk) values(1,1);


--LOGIN = {CODLOGIN, USUARIO, SENHA, CODFUNCIONARIOFK}
create table login (
	codlogin INT NOT NULL IDENTITY PRIMARY KEY,
...
--CONTROLELOGSISTEMA = {CODCONTROLE, CODLOGINFK, DATALOG, HORA}
create table controlelogsistema(
	codcontrole INT NOT NULL IDENTITY PRIMARY KEY,
	codlogin_fk integer not null references login(codlogin) ON DELETE CASCADE ON UPDATE CASCADE,
...
create table acesso(
	codacesso INT NOT NULL IDENTITY PRIMARY KEY,
...
--ITENSTELEFONELOJA={CODLOJAFK, CODTELEFONEFK}
...
insert into itenstelefoneloja(codtelefone_fk,codloja_fk) values (1,1);


--MARCA={CODMARCA, NOMEMARCA}
create table marca(
	codmarca INT NOT NULL IDENTITY PRIMARY KEY,
...
create table tipo(
	codtipo INT NOT NULL IDENTITY PRIMARY KEY,
	nometipo varchar(80) not null unique
);



--PRODUTO = {CODPRODUTO, NOMEPRODUTO, QUANTIDADE, VALOR, CODMARCAFK,CODTIPOFK }
create table produto (
	codproduto INT NOT NULL IDENTITY PRIMARY KEY,
	nomeproduto varchar(80) not null unique,
	quantidade numeric(10,2) default (0),
	valor numeric(10,2) not null check (valor > 0),
	codmarca_fk integer not null references marca(codmarca) ON DELETE CASCADE ON UPDATE CASCADE,
	codtipo_fk integer not null references tipo(codtipo) ON DELETE CASCADE ON UPDATE CASCADE
);
	
--IMAGENS={CODIMAGEN, IMAGEM, DESCRICAO, CODPRODUTOFK}
create table imagem(
	codimagem INT NOT NULL IDENTITY PRIMARY KEY,
	imagem varbinary(max) not null,
	descricao text not null,
	codproduto_fk integer not null references produto(codproduto) ON DELETE CASCADE ON UPDATE CASCADE
	);



--FORNECEDOR={CODFORNECEDOR, NOMEFORNECEDOR, NUMEROCASA, CORUAFK,CODBAIRROFK, CODCEPFK, CODCIDADEFK}
create table fornecedor (
	codfornecedor INT NOT NULL IDENTITY PRIMARY KEY,
...
--ITENSTELEFONEFORNECEDOR={CODTELEFONEFK, CODFORNECEDORFK}
create table itenstelefonefornecedor (
	codtelefone_fk integer references telefone(codtelefone) ON DELETE CASCADE ON UPDATE CASCADE,
	codfornecedor_fk integer references fornecedor(codfornecedor) ON DELETE CASCADE ON UPDATE CASCADE,
	primary key(codtelefone_fk, codfornecedor_fk)
);



--COMPRAPRODUTO = {CODCOMPRA, DATACOMPRA, CODFORNECEDORFK,CODFUNCIONARIOFK}
create table compraproduto(
	codcompra INT NOT NULL IDENTITY PRIMARY KEY,
	datacompra date not null,
	codfornecedor_fk integer not null references fornecedor(codfornecedor) ON DELETE NO ACTION ON UPDATE NO ACTION,
	codfuncionario_fk integer not null references funcionario(codfuncionario) ON DELETE NO ACTION ON UPDATE NO ACTION
);



--ITENSCOMPRAPRODUTO = {CODCOMPRAFK, CODPRODUTOFK, QUANTIDADE, VALORC}
...
create table situacao (
	codsituacao INT NOT NULL IDENTITY PRIMARY KEY,
...
--VENDAPRODUTO = {CODVENDA, DATAVENDA, CODCLIENTEFK, CODFUNCIONARIOFK}
...
--ITENSVENDAPRODUTO = {CODVENDAFK,CODPRODUTOFK, QUANTIDADE, VALOR}
create table itensvendaproduto (
	codvenda_fk integer not null references vendaproduto(codvenda) ON DELETE CASCADE ON UPDATE CASCADE,
	codproduto_fk integer not null references produto(codproduto) ON DELETE CASCADE ON UPDATE CASCADE,
	quantidade numeric (10,2) not null check (quantidade >0),
	valor numeric (10,2) not null check(valor >0),
